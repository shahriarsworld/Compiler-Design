...
set<string> computeFirst(const vector<string> &alpha) {
    ...
    for (auto X: alpha) {
        if (isTerminal(X)) {
            result.insert(X); eps=false; break;
        } else {
            for (auto y: FIRST[X]) if (y!="ε") result.insert(y);
            if (FIRST[X].find("ε")==FIRST[X].end()) { eps=false; break; }
        }
    }
    ...
    while (changed) {
        ...
        for (auto &[lhs,prods]:grammar) {
            for (auto prod:prods) {
                ...
                for (auto x:f) if (FIRST[lhs].insert(x).second) changed=true;
            }
        }
    }
}
...
bool sameCoreItem(const Item &a,const Item &b) {
    return a.lhs==b.lhs && a.rhs==b.rhs && a.dotPos==b.dotPos;
}
                ...
                if (!isTerminal(B)) {
                    ...
                    for (auto prod:grammar[B]) {
                        for (auto a:lookaheads) {
                            ...
                            if (!exists) { newItems.push_back(newItem); added=true; }
                        }
                    }
                }
            }
...
vector<Item> GOTOfunc(vector<Item> I,string X) {
    ...
    for (auto &it:I) {
        if (it.dotPos<it.rhs.size() && it.rhs[it.dotPos]==X) {
            Item moved=it; moved.dotPos++; J.push_back(moved);
        }
    }
    return closure(J);
}
    ...
    for (int i=0;i<states.size();i++) {
        ...
        for (auto X:symbols) {
            ...
            for (int j=0;j<states.size();j++) {
                if (sameState(states[j].items,newState)) { exists=true; idx=j; break; }
            }
            if (!exists) {
                states.push_back({newState,{}}); idx=states.size()-1;
            }
...
// Merge states with same LR(0) core for LALR
void mergeStates() {
    vector<State> merged;
    vector<int> mapping(states.size(),-1);

    for (int i=0;i<states.size();i++) {
        if (mapping[i]!=-1) continue;
        vector<Item> core;
        for (auto &it:states[i].items) {
            Item coreItem=it; coreItem.lookahead=""; // ignore lookahead
            core.push_back(coreItem);
        }

        vector<int> mergeList={i};
        for (int j=i+1;j<states.size();j++) {
            vector<Item> coreJ;
            for (auto &it:states[j].items) {
                Item coreItem=it; coreItem.lookahead="";
                coreJ.push_back(coreItem);
            }
            if (core.size()==coreJ.size()) {
                bool eq=true;
                for (auto &a:core) {
                    bool found=false;
                    for (auto &b:coreJ)
                        if (sameCoreItem(a,b)) found=true;
                    if (!found) {eq=false; break;}
                }
                if (eq) mergeList.push_back(j);
            }
        }

        vector<Item> mergedItems=states[i].items;
        for (auto j:mergeList) {
            mapping[j]=merged.size();
            for (auto &it:states[j].items) mergedItems.push_back(it);
        }
        merged.push_back({mergedItems,{}});
    }

    for (int i=0;i<merged.size();i++) {
        for (int j=0;j<states.size();j++) {
            if (mapping[j]==i) {
                for (auto &tr:states[j].transitions)
                    merged[i].transitions[tr.first]=mapping[tr.second];
            }
        }
    }
...
int main() {
    ...
    for (int i=0;i<n;i++) {
        ...
        stringstream ss(rhs); string prod;
